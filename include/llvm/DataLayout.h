...
public:
  ...
  explicit DataLayout(StringRef LayoutDescription, unsigned _BitsPerBytes = 8)
    : ImmutablePass(ID) {
    std::string errMsg = parseSpecifier(LayoutDescription, this, _BitsPerBytes);
  ...
  unsigned getPointerSizeInBits(unsigned AS = 0)    const {
    return getPointerSize(AS) * BitsPerByte;
  }
  /// Size examples:
  ///
