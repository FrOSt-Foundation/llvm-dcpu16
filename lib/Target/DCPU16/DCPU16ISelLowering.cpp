...
getRegForInlineAsmConstraint(const std::string &Constraint,
  ...
  return TargetLowering::getRegForInlineAsmConstraint(Subtarget.getRegisterInfo(), Constraint, VT.getSimpleVT());
}
...
DCPU16TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  const DebugLoc dl                     = CLI.DL.getDebugLoc();
...
//                             DAG.getConstant(NumBytes, getPointerTy(*TD), true),
//                             DAG.getConstant(0, getPointerTy(*TD), true),
//                             InFlag);
  InFlag = Chain.getValue(1);

  // Handle result values, copying them out of physregs into vregs that we
  // return.
  */return Chain;//LowerCallResult(Chain, InFlag, CallConv, isVarArg, Ins, dl,
            //             DAG, InVals);
}
...
SDValue DCPU16TargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  //                                            getPointerTy(), Offset);
  return Op;//DAG.getNode(DCPU16ISD::Wrapper, Op.getDebugLoc(),
            //         getPointerTy(), Result);
}
...
SDValue DCPU16TargetLowering::LowerExternalSymbol(SDValue Op,
  ...
  //SDValue Result = DAG.getTargetExternalSymbol(Sym, getPointerTy(*TD));

  return Op;//DAG.getNode(DCPU16ISD::Wrapper, dl, getPointerTy(), Result);
}
...
SDValue DCPU16TargetLowering::LowerBlockAddress(SDValue Op,
  ...
  //SDValue Result = DAG.getBlockAddress(BA, getPointerTy(*TD), /*isTarget=*/true);
...
SDValue DCPU16TargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return Op;//DAG.getNode(DCPU16ISD::BR_CC, dl, Op.getValueType(),
...
SDValue DCPU16TargetLowering::LowerSELECT_CC(SDValue Op,
  ...
  return Op;//DAG.getNode(DCPU16ISD::SELECT_CC, dl, VTs, &Ops[0], Ops.size());
}
...
SDValue DCPU16TargetLowering::LowerSIGN_EXTEND(SDValue Op,
  ...
  return Op;//DAG.getNode(ISD::SIGN_EXTEND_INREG, dl, VT,
...
DCPU16TargetLowering::getReturnAddressFrameIndex(SelectionDAG &DAG) const {
  ...
  return DAG.getFrameIndex(ReturnAddrIndex, getPointerTy(*TM.getDataLayout()));
}
...
SDValue DCPU16TargetLowering::LowerRETURNADDR(SDValue Op,
  ...
  if (Depth > 0) {
    ...
    return Op; //DAG.getLoad(getPointerTy(), dl, DAG.getEntryNode(),
...
SDValue DCPU16TargetLowering::LowerROT(SDValue Op,
  ...
  return Op;//DAG.getNode(ISD::OR, dl, VTs2, Ops2, array_lengthof(Ops2));
}
...
SDValue DCPU16TargetLowering::LowerMUL_LOHI(SDValue Op,
  ...
  *///SDValue Ops2[] = {Lo, Hi};
  return Op;//DAG.getMergeValues(Ops2, dl);
}
...
SDValue DCPU16TargetLowering::LowerJumpTable(SDValue Op,
  ...
  return Op;//DAG.getNode(DCPU16ISD::Wrapper, dl, MVT::i16, TJT);
}
