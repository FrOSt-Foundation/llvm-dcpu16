...
bool DCPU16Peephole::runOnMachineFunction(MachineFunction &MF) {
  QII = static_cast<const DCPU16InstrInfo *>(MF.getTarget().getMCInstrInfo());
  QRI = static_cast<const DCPU16RegisterInfo *>(MF.getTarget().getMCRegisterInfo());
  MRI = &MF.getRegInfo();
