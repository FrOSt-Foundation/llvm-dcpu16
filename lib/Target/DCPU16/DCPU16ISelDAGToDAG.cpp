...
bool DCPU16DAGToDAGISel::SelectAddr(SDValue N,
  ...
  Base  = (AM.BaseType == DCPU16ISelAddressMode::FrameIndexBase) ?
    CurDAG->getTargetFrameIndex(AM.Base.FrameIndex, TLI->getPointerTy(MF->getDataLayout())) :
    AM.Base.Reg;

  SDLoc DL = SDLoc(N);

  if (AM.GV)
    Disp = CurDAG->getTargetGlobalAddress(AM.GV, DL,
                                          MVT::i16, AM.Disp,
                                          0/*AM.SymbolFlags*/);
  else if (AM.CP)
    ...
    Disp = CurDAG->getTargetConstant(AM.Disp, DL, MVT::i16);
...
void DCPU16DAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::FrameIndex: {
    ...
    if (Node->hasOneUse()) {
      CurDAG->SelectNodeTo(Node, DCPU16::ADD16ri, MVT::i16,
                                  TFI, CurDAG->getTargetConstant(0, DL, MVT::i16));
      return;
    }
    CurDAG->getMachineNode(DCPU16::ADD16ri, DL, MVT::i16,
                                  TFI, CurDAG->getTargetConstant(0, DL, MVT::i16));
    return;
  }
  }
