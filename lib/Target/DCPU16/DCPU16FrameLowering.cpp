...
// returns true if the specified function should have a dedicated frame pointer
// register. This is true if the function has variable sized allocas, if it needs
// dynamic stack realignment, if frame pointer elimination is disabled or if the
...
void DCPU16FrameLowering::emitPrologue(MachineFunction &MF, MachineBasicBlock &MBB) const {
  ...
  const DCPU16InstrInfo &TII =
    *static_cast<const DCPU16InstrInfo*>(MF.getTarget().getMCInstrInfo());
...
DCPU16FrameLowering::spillCalleeSavedRegisters(MachineBasicBlock &MBB,
  ...
  const TargetInstrInfo &TII = *static_cast<const DCPU16InstrInfo*>(MF.getTarget().getMCInstrInfo());
